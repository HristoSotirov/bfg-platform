...
export class AthleteDetailPageComponent implements OnInit, OnDestroy {
  ...
  private loadAthlete(uuid: string): void {
    ...
    this.athletesService
      ...
      .pipe(
        catchError((err) => {
          this.error = err?.error?.message || 'Грешка при зареждане на атлета';
          this.loading = false;
          this.cdr.markForCheck();
          return of(null);
        }),
      ...
      .subscribe({
        next: (athlete) => {
          ...
          this.loading = false;
          if (!athlete) {
            this.error = this.error || 'Атлетът не е намерен.';
            this.cdr.markForCheck();
          } else {
  ...
  saveStatus(uuid: string): void {
    ...
    this.accreditationsService.updateAccreditationStatus(uuid, { status: newStatus }).subscribe({
      ...
      error: (err: any) => {
        this.saving = false;
        this.error = err?.error?.message || 'Грешка при запазване на статус';
        this.cdr.markForCheck();
      },
    });
  }
  ...
  uploadPhoto(file: File): void {
    ...
    if (!this.canUploadPhoto) {
      this.photoError = 'Качването не е позволено.';
      this.cdr.markForCheck();
      return;
    }
