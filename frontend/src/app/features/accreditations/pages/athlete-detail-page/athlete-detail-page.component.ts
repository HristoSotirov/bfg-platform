...
type Tab = 'details' | 'accreditation-history';
...
export class AthleteDetailPageComponent implements OnInit, OnDestroy {
  private destroy$ = new Subject<void>();

  athleteUuid: string | null = null;
  athlete: AthleteDto | null = null;
  loading = true;
  ...
  editingStatusUuid: string | null = null;
  deletingAccreditationUuid: string | null = null;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      ...
      const tab = this.isValidTab(tabParam) ? tabParam as Tab : 'details';

      if (uuid && uuid !== this.athleteUuid) {
        this.athleteUuid = uuid;
        this.activeTab = tab;
        this.loadAthlete(uuid);
  ...
  private isValidTab(tab: string | null): boolean {
    return tab === 'details' || tab === 'accreditation-history';
  }
  ...
  setTab(tab: Tab): void {
    ...
    this.activeTab = tab;
    this.router.navigate(['/athletes', this.athleteUuid, tab]);
    this.cdr.markForCheck();
  }
  ...
  get fullName(): string {
    if (!this.athlete) return this.translateService.instant('accreditations.athleteDetailPage.defaultName');
    const name = [this.athlete.firstName, this.athlete.middleName, this.athlete.lastName]
      .filter(Boolean).join(' ');
    return name || this.translateService.instant('accreditations.athleteDetailPage.defaultName');
  }
  ...
  private loadAthlete(uuid: string): void {
    this.loading = true;
    this.error = null;
    this.fullHistory = [];
    ...
    this.athletesService
      .getAthleteByUuid(uuid)
      .pipe(
        catchError((err) => {
          this.error = err?.error?.message || this.translateService.instant('accreditations.athleteDetailPage.errors.loadFailed');
          this.loading = false;
          this.cdr.markForCheck();
          return of(null);
        }),
      ...
      .subscribe({
        next: (athlete) => {
          ...
          this.loading = false;
          if (!athlete) {
            this.error = this.error || this.translateService.instant('accreditations.athleteDetailPage.errors.notFound');
            this.cdr.markForCheck();
          } else {
  ...
  startEditingStatus(uuid: string): void {
    this.editingStatusUuid = uuid;
    this.cdr.markForCheck();
  }

  cancelEditingStatus(): void {
    this.statusChanges.delete(this.editingStatusUuid!);
    this.editingStatusUuid = null;
    this.cdr.markForCheck();
  }

  saveStatus(uuid: string): void {
    const newStatus = this.statusChanges.get(uuid);
    const orig = this.fullHistory.find((acc) => acc.uuid === uuid);
    if (!newStatus || !orig || orig.status === newStatus) {
      this.editingStatusUuid = null;
      this.statusChanges.delete(uuid);
      this.cdr.markForCheck();
      return;
    }
    this.saving = true;
    this.cdr.markForCheck();
    this.accreditationsService.updateAccreditationStatus(uuid, { status: newStatus }).subscribe({
      next: () => {
        this.saving = false;
        this.editingStatusUuid = null;
        this.statusChanges.delete(uuid);
      ...
      error: (err: any) => {
        this.saving = false;
        this.error = err?.error?.message || this.translateService.instant('accreditations.athleteDetailPage.errors.statusSaveFailed');
        this.cdr.markForCheck();
      },
    });
  }
  ...
  getDeletingAccreditationName(): string {
    ...
    return acc?.accreditationNumber ? this.translateService.instant('accreditations.athleteDetailPage.deletingAccreditationName', { number: acc.accreditationNumber }) : '';
  }
  ...
  executeDeleteAccreditation(): void {
    if (!this.deletingAccreditationUuid) return;
    this.deleting = true;
    this.cdr.markForCheck();
    this.accreditationsService.deleteAccreditation(this.deletingAccreditationUuid).subscribe({
      next: () => {
        this.deleting = false;
        this.showDeleteAccreditationConfirm = false;
        this.deletingAccreditationUuid = null;
      ...
      error: (err: any) => {
        this.deleting = false;
        this.deleteAccreditationError = err?.error?.message || this.translateService.instant('accreditations.athleteDetailPage.errors.deleteAccreditationFailed');
        this.cdr.markForCheck();
      },
    });
  }
  ...
  executeDeleteAthlete(): void {
    if (!this.athleteUuid) return;
    this.deleting = true;
    this.cdr.markForCheck();
    this.athletesService.deleteAthlete(this.athleteUuid).subscribe({
      next: () => {
        this.deleting = false;
      ...
      error: (err: any) => {
        this.deleting = false;
        this.deleteAthleteError = err?.error?.message || this.translateService.instant('accreditations.athleteDetailPage.errors.deleteAthleteFailed');
        this.cdr.markForCheck();
      },
    });
  }
  ...
  uploadPhoto(file: File): void {
    ...
    if (!this.canUploadPhoto) {
      this.photoError = this.translateService.instant('accreditations.athleteDetailsDialog.photoErrors.uploadNotAllowed');
      this.cdr.markForCheck();
      return;
    }
