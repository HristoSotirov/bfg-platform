...
public final class ScoringRuleQueryAdapter {
    private static final Sort DEFAULT_SORT = Sort.by(new Sort.Order(Sort.Direction.ASC, "placement"));
    private static final Map<String, Sort.Order> ORDER_MAP = Map.ofEntries(
            Map.entry("placement_asc", new Sort.Order(Sort.Direction.ASC, "placement")),
            Map.entry("placement_desc", new Sort.Order(Sort.Direction.DESC, "placement")),
    ...
    private ScoringRuleQueryAdapter() {
        throw new IllegalStateException("Utility class");
    }

    public static Specification<ScoringRule> parseFilter(String filter) {
        EnhancedFilterExpressionParser.ParseResult<ScoringRule> result = parseFilter(filter, null);
        return result.getSpecification();
    }

    public static EnhancedFilterExpressionParser.ParseResult<ScoringRule> parseFilter(String filter, Set<String> requestedExpand) {
    ...
    private static class EnhancedPredicateBuilder implements EnhancedFilterExpressionParser.EnhancedPredicateBuilder<ScoringRule> {
        @Override
        public Predicate build(Root<ScoringRule> root, CriteriaBuilder cb, String field, String op, String valueRaw) {
            return buildPredicate(root, cb, field, op, valueRaw);
        }

        @Override
        public Predicate buildRange(Root<ScoringRule> root, CriteriaBuilder cb, String field, String minValue, String maxValue) {
            return switch (field) {
                case "placement" ->
        ...
        @Override
        public Predicate buildIn(Root<ScoringRule> root, CriteriaBuilder cb, String field, List<String> values) {
            return switch (field) {
    ...
    public static Specification<ScoringRule> parseSearch(String search) {
    ...
    private static Predicate buildPredicate(Root<ScoringRule> root, CriteriaBuilder cb, String field, String op, String valueRaw) {
        return switch (field) {
            case "scoringSchemeId" -> QueryAdapterHelpers.uuidPredicate(root, cb, "scoringSchemeId", op, valueRaw);
            case "placement" -> QueryAdapterHelpers.integerPredicate(root, cb, field, op, valueRaw);
